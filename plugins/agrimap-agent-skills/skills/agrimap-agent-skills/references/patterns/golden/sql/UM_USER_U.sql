...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS (
			...
			FROM [agrimap_app].[UM_USER]
			WHERE [USER_ID] <> @PI_USER_ID
				AND UPPER(LTRIM(RTRIM([EMAIL]))) = UPPER(LTRIM(RTRIM(@PI_EMAIL)))
				AND [DEL_FLAG] = 0
		)
		BEGIN
		...
		SET
			[PASSWORD] = CASE
				WHEN @PI_PASSWORD IS NOT NULL AND LTRIM(RTRIM(@PI_PASSWORD)) <> ''
				THEN @PI_PASSWORD
				ELSE [PASSWORD]
			END,
			...
			[IMAGE] = CASE
				...
				ELSE [IMAGE]
			END,
			[FORCE_CHANGE_PASSWORD] = ISNULL(@PI_FORCE_CHANGE_PASSWORD, [FORCE_CHANGE_PASSWORD]),
			[SOURCE] = CASE
