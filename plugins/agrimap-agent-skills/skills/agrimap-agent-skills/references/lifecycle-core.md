...
- `light`: help, history, read-only queries, or bounded work with at most three product artifacts, one logical contract/writer, no delegation, no durable-audit request, and no regulated trigger.
...
- `light`: do not identify/persist the requester, emit a receipt, or create `.agrimap-agent` state. Use one agent, self-review, targeted proportional verification, and a concise direct result.
