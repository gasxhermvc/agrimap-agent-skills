...
Generated `$agm-*`/`/agm-*` aliases use progressive disclosure: they read [runtime-core.md](references/runtime-core.md), this glossary, and one generated `references/operations/<operation>.md` entrypoint. They must not load this umbrella file during a normal alias invocation. Read the umbrella only for a direct umbrella request, an unknown operation, or a missing/corrupt compact entrypoint.
...
**Work with the harness, not against it.** Current Codex releases enable subagent workflows by default and expose activity in the app, CLI, and IDE; the former pre-native Codex limitation is obsolete. A spawned agent receives its delegated prompt/configuration, not the whole reasoning context: inline the specific rules, pattern excerpts, file/line targets, and output contract it needs; never tell it to reload the umbrella or reference tree. Before spawning, tell the requester each descriptive agent label, bounded task, expected output, and native inspection path—open the agent thread in the app, use `/agent` in Codex CLI, or expand the IDE background-agent panel. Native thread activity is the primary status channel.

**Never wait silently.** While subagents are active, continue safe independent Leader work or inspect their status. When waiting is necessary, use bounded wait/poll intervals of at most 60 seconds and give the requester a concise `running|completed|blocked` update at least every 60 seconds; do not emit repeated “Waiting for subagent…” with no task/agent detail. Use `.agrimap-agent/runtime/progress/<task-id>.jsonl` only as a named fallback when the active surface cannot expose native agent threads. If the current surface genuinely has no callable subagent capability or it is disabled by policy, state that verified limitation and run Leader → executor → QA as sequential passes without re-reading unchanged references.

Assign the Leader role to a model that resolves to the required capability profile on the active host. Before delegation, resolve the configurable model label and record the actual model separately. For implementation tasks, assign final verification to an independent verification-only QA subagent/context; the Leader must not QA its own implementation or edit a QA finding inside the task being verified. Use no more than five active subagents. Before delegation, specify and verify `workspace_need` and create a file-ownership map. In one integration wave, one file and one logical contract have one writer model; QA may not modify product artifacts but may write only its assigned workflow evidence. Assign each subagent a descriptive display label/nickname, bounded task, model profile, model label, actual execution identity, workspace need, required skill excerpts, file/line targets, forbidden files, verification steps, native inspection instructions, and structured result contract.
