...
ถ้าต้องการดูวิธีใช้ ให้เติม `-h` หรือ `--help`; การเรียกนี้เป็น `light` และยังบันทึก task, memory และ log แบบกระชับ:
...
- `light`: help, history/read-only query และงาน bounded ใช้ agent เดียว + self-review + targeted verification พร้อม brief/checklist/memory/log/result แบบกระชับ แต่ไม่เปิด verifier แยกหรือสร้าง `qa.md`;
...
ทุก depth ต้องบันทึก requester, task, memory และ log; `standard`/`regulated` ใช้ activation receipt ที่ละเอียดขึ้น ส่วน `light` ใช้ receipt กระชับ:
...
3. ตรวจและ approve `brief.md`, acceptance `checklist.md`, และ executor prompt; ขั้นนี้ยังไม่มี `qa.md` หรือ `result.md`.
...
`agm-create-prompt` เดินเป็น stage: **intake sweep → เสนอแนวทางพร้อม counter-argument → ปรับ Thesis → checklist ความเข้าใจ (ต้อง approve) → generate**. มันกวาดบริบทจากบทสนทนาและ `.agrimap-agent/` (memory, decisions, knowledge) ให้เอง ไม่ต้องเล่าซ้ำ และจบที่ `brief.md` + acceptance `checklist.md` + draft prompt พร้อม non-terminal scope checkpoint — ไม่เริ่มรัน, ไม่สร้าง `qa.md`/`result.md`, และไม่ปิด task เอง:
