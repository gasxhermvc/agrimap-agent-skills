...
These definitions are normative across the umbrella skill, references, templates, prompts, logs, and generated provider copies. A narrower document may add constraints but must not redefine these terms.
...
Every task brief and generated prompt records `requested_by`, `requester_authority`, `decision_owner`, and `authority_evidence`. If a material decision is required while authority is `requester-only` or `unknown`, safe product-read-only investigation may continue, but the affected decision and any dependent write must stop.
...
- **Workflow artifact**: files used only to run and audit this workflow, including `.agrimap-agent/tasks/**`, prompts, memory, decisions, knowledge, logs, and ignored runtime heartbeat/session files.
...
- **Verification-only QA**: the preferred name for the former shorthand “read-only QA.” QA has read-only access to product artifacts and must not fix or mutate them, but may write only its assigned workflow artifacts: `qa.md`, its own heartbeat, and its own checkpoint/log evidence. It may run only non-mutating verification commands allowed by the QA contract.

## Task size, decisions, and checkpoints

- **Checkpoint unit**: exactly one durable state transition: task creation; an authorized scope/decision change; completion of one logical implementation batch; integration of one delegated handoff; one verification or QA outcome; or a terminal task transition. Append one checkpoint per unit. A file read, tool call, heartbeat, unchanged retry, or conversational update is not a checkpoint unit.
