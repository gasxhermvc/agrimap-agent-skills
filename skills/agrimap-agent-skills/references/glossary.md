...
Every standard/regulated task brief records `requested_by`, `requester_authority`, `decision_owner`, and `authority_evidence`; light work records requester attribution in runtime identity, memory, and daily logs without a task brief. Generated instructions record the same authority fields when that operation produces them. If a material decision is required while authority is `requester-only` or `unknown`, safe product-read-only investigation may continue, but the affected decision and any dependent write must stop.
...
- **Workflow artifact**: files used only to run and audit this workflow, including tracked tasks, raw requester prompts, generated instructions, reports, memory, decisions, knowledge, logs, and ignored runtime/session files. These categories have separate ownership and directories.
