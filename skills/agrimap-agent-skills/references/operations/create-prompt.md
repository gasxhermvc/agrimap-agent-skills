...
- Deliverable: tracked brief.md and acceptance checklists.md plus .agrimap-agent/instructions/YYYY-MM/<task-id>/<role>.prompt.md; no implementation, qa.md, or result.md
...
1. Own the tracked feature contract: run staged elicitation, write the approved brief.md and acceptance checklists.md, build one execution SoT per task under instructions/, and create separate Leader/executor/QA instructions when implementation is delegated.
2. Generation ends in draft and a non-terminal scope-decision checkpoint; never execute the generated instruction, create qa.md, create result.md, or complete the implementation task in this operation.
