...
```text
Requester: /agm-prompt provider=codex สร้าง project template ใหม่ให้หน่อย ขอชื่อ publisher
...
Domain `create`, `edit`, and explicit FE/BE `test` actions embed bounded supporting analysis/design; do not require a separate analysis task first. Unit-test generation is guided by an embedded test-decision capability: required for complex/high-risk behavior, recommended for simpler logic when value is plausible, and not-applicable for product-read-only or non-executable changes.

- Unit tests: embedded FE/BE test-decision classifies risk in read or authorized write work. Read-only recommends; authorized implementation may require regression tests. Explicit `action=test` or a clear request supplies write intent. If coverage is unspecified, list choices for `approve`, `all`, or numbers such as `1 2 5`; if behaviors are named, create only those tests.
- Features: the objective is Tier 1 and never guessed. Infer placement from evidence, propose the smallest slice plan (files to create or modify), and confirm once. Build directly only when the entire slice remains light; when it needs more than three product artifacts, tracking, delegation, separate QA, or material owner decisions, route the unexecuted slice to `agm-prompt` and later execute an approved version through `agm-exec`.
...
A full rewrite is not a refactor. Route "เปลี่ยนทั้งหมด" requests to `agm-plan`/`agm-architect`, then use `agm-fe|agm-be|agm-sql action=create|edit` only for a light slice or `agm-prompt` followed by `agm-exec` for tracked execution, so `action=refactor` never becomes a back door for rebuilding a system.
