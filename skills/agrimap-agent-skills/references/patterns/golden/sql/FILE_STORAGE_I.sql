...
BEGIN
	...
	BEGIN TRY
		...
		SET @V_ROWS = CURSOR FOR SELECT [VALUE] FROM agrimap_app.FN_SPLIT(@PI_FILE_LISTS, @V_SPLIT_ROW);
		OPEN @V_ROWS;
		...
		BEGIN
			...
			SET @SESSION_USER_ID = '';
			SET @V_COLS = CURSOR FOR SELECT [VALUE] FROM agrimap_app.FN_SPLIT(@V_ROW_VALUE, @V_SPLIT_COL);
			OPEN @V_COLS;
			...
			BEGIN
				...
				BEGIN
					IF (ISNUMERIC(@V_COL_VALUE) = 1)
						SET @SIZE = CAST(@V_COL_VALUE AS DECIMAL(28, 4));
					ELSE
						THROW 50001, 'invalid_file_size', 1;
				END
				IF (@I = 9) SET @DATE_CREATED = DATEADD(SECOND, CAST(@V_COL_VALUE AS BIGINT) / 1000, '19700101 00:00:00:000');
			...
			IF (ISNULL(@FILE_ID, '') = '' OR ISNULL(@FILE_NAME, '') = '' OR ISNULL(@FILE_RENAME, '') = '' OR ISNULL(@EXTENSION, '') = '' OR ISNULL(@MIME_TYPE, '') = '' OR
			ISNULL(@FILE_SOURCE, '') = '' OR @DATE_CREATED IS NULL OR ISNULL(@SESSION_USER_ID, '') = '')
				THROW 50001, 'invalid_parameter', 1;
