...
- `plugins/agrimap-agent-skills/skills/agm-*` are generated thin aliases from `config/operations.json`; they route back to the mirrored umbrella skill and contain no workflow policy of their own.
...
`package.json` is the only package-version source of truth. `npm run sync` propagates that version to the Codex manifest, Claude manifest/marketplace, and Gemini manifest while rebuilding the plugin umbrella mirror, all thin aliases, and provider adapters from the canonical umbrella and `config/operations.json`. Never edit the plugin umbrella mirror, generated alias skills, Gemini command files, or generated manifest versions directly. Before a public release, resolve the golden-example rights/license decision, update `package.json`, add the release entry to `CHANGELOG.md`, then rerun sync, tests, and validation.
