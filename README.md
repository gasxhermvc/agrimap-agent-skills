...
## Lifecycle and first tracked use

Each dedicated operation declares one lifecycle in `config/operations.json`. `stateless` creates no task state. A `lightweight-eligible` operation uses the lightweight lane when it stays within three product artifacts, one logical contract/writer, and no audit, delegation, material decision, sensitive boundary, or release boundary; that lane has no requester persistence, activation receipt, `.agrimap-agent` artifacts, or separate QA. Scope growth promotes the work to tracked before the next product write. `tracked-only` always uses the tracked lane. The normative selector lives in [`runtime-core.md`](skills/agrimap-agent-skills/references/runtime-core.md).

On the first tracked interaction, the Leader resolves who is requesting the work. In a multi-person project there is no shared `owner.json`.

Requester identity and decision authority are separate. Every tracked task records requester authority (`owner|delegated|requester-only|unknown`), the decision owner, and authority evidence; a requester without owner/delegated authority cannot approve a material trade-off. Normative terms live in [`glossary.md`](skills/agrimap-agent-skills/references/glossary.md).
...
If tracked work lacks a confirmed human, the hook instructs the agent to resolve that identity. Lightweight/stateless work does not ask solely for attribution. The hook never copies the requester from the latest shared log.
...
| `agm-qa` | canonical tracked QA in a separate product-read-only context |
...
Only tracked work enters the canonical QA and completion flow. Its verifier boundary, evidence permissions, bounded correction cycle, terminal handling, and full-QA triggers are defined once in [`qa-and-done.md`](skills/agrimap-agent-skills/references/qa-and-done.md); consumers must link to that contract rather than restate it.

Tracked `complete` remains schema-validated and requires the checked checklist, QA evidence/identity, delivery boundary, and Outstanding items. Commit/publish/release boundaries require full QA.
